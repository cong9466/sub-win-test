...
// =========================================================================
// W2 的子类化过程（拦截 W2 背景的鼠标消息，实现拖拽和边界限制）
// =========================================================================
...
void AttachExternalWindow(HWND hMain, HWND hW2) {
    ...
    // 4. 调整初始位置并显示
    SetWindowPos(hW2, NULL, 50, 80, 240, 180, SWP_NOZORDER | SWP_FRAMECHANGED);
...
// W2（外部小窗口）的独立消息处理过程
// =========================================================================
LRESULT CALLBACK W2WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
    case WM_CREATE: {
        // 在 W2 内部创建一个按钮
        CreateWindowW(L"BUTTON", L"hello",
            WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
            70, 60, 100, 35, hWnd, (HMENU)2001, g_hInst, nullptr);
        break;
    }
    case WM_COMMAND: {
        // 处理 W2 内部按钮的点击事件
        if (LOWORD(wParam) == 2001) {
            MessageBoxW(hWnd, L"hello,world", L"W2 提示", MB_OK | MB_ICONINFORMATION);
        }
        break;
    }
    default:
...
HWND SimulateCreateExternalWindow() {
    WNDCLASS wc = { 0 };
    wc.lpfnWndProc = W2WndProc; // 指向 W2 专属的窗口过程，不再使用 DefWindowProc
    ...
    HWND hWnd = CreateWindow(L"ExternalSimulatedClass", L"", 
        WS_POPUP | WS_VISIBLE, 0, 0, 240, 180, NULL, NULL, g_hInst, NULL);
    return hWnd;
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_SIZE: {
        // 健壮性处理：W1 被缩小导致 W2 超出边界时，把 W2 挤回来
        if (g_hExtWnd) {
            RECT rcParent, rcW2;
